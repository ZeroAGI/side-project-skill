...
| 搜索通道 | ⚠️ **全局故障**：63 次 WebSearch 中 30 次（48%）查询串被本地 harness 内部文本顶替，返回同一组无关结果（首条恒为同一篇与查询无关的第三方博客）。**工具侧污染，非外部注入攻击**。受影响面覆盖 12 个 agent，含 Reddit 商业组（该组因此 0 信号）、会议组、WAIC 组、融资组、中文组 |
